...
def make_report(buz_class_id,course_code,subject_show_id,dict_index='3',user='15600905550',password='123456'):
    client = requests.session()
    class_id=add_class(time.time(), 'A店', '12', '12', course_code, subject_show_id,dict_index=dict_index)
    ...
    for x in range(len(data['classDataList'])):
            ...
            data['classDataList'][x]['userUuid'] = uuid_idct[dict_index][x]
    ...
    #  make_report(buz_class_id=218604,course_code='JZX2.0.4.1',subject_show_id='1',dict_index='4')
     for  x in  range(99):
       #  kk_test.table('user_daily_weight').updata({'weight':x},id=)
         make_report(buz_class_id=331096, course_code='JZX2.0.2.5', subject_show_id='1', dict_index='1')
