...
def  appium_data(context,Q,course,x):
    ...
    use_mysql.add_class(add_time=1200, store_name=None, user_number='12', classes_checkin_number='12',
    ...
    for x in driver.find_elements_by_id(称重按钮):
        x.click()
...
def test_case(context):
    ...
    course_list=my_db.table('web_platform_course').select(name__LIKE='燃脂循环').all()
    rult_list=[]
    sigu=False
    Q = Queue()
    Q_pid = Queue()
    for  course  in  course_list:
        for  x  in range(1,int(course['subject_total'])):
            L.acquire()
            p1 = Process(target=appium_data, args=(context,Q,course,x))
